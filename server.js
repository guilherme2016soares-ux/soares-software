...
// ===== TRAVA MÍNIMA DE ACESSO =====
// Exige um token secreto (variável de ambiente TOKEN_SISTEMA, configurada no painel da Render)
// em toda rota /api/*. Sem essa variável configurada, TODA requisição é recusada (falha fechada,
// nunca aberta) — evita que um deploy sem a variável ainda configurada deixe a API sem proteção.
function exigirToken(req, res, next) {
  const esperado = process.env.TOKEN_SISTEMA;
  const recebido = req.headers['x-sistema-token'];
...
app.get('/api/data', exigirToken, async (req, res) => {
  try {
...
app.post('/api/data', exigirToken, async (req, res) => {
  try {
...
app.post('/api/upsert', exigirToken, async (req, res) => {
...
app.post('/api/delete', exigirToken, async (req, res) => {
  const { ops } = req.body || {};
